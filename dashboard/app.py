...
if requested_workspace in {"Home", "Priorities", "Asset review", "Work verification", "Ask AI"}:
    st.session_state["workspace"] = requested_workspace
if st.session_state.get("workspace") not in {"Home", "Priorities", "Asset review", "Work verification", "Ask AI"}:
...
with st.sidebar:
    ...
    workspace = st.radio(
        "Workspace",
        ["Home", "Priorities", "Asset review", "Work verification", "Ask AI"],
        key="workspace",
...
if workspace == "Priorities":
    ...
    _section(
        "Ask AI",
        "Discuss the selected asset's visible evidence, recommendation, and maintenance workflow.",
        "Human decision support",
    )
    left, right = st.columns([1.3, 1])
    with left:
        st.markdown(
            f"""<div class='lp-panel'>
              <div class='lp-panel-label'>Current asset context</div>
              <div class='lp-panel-value'>{_h(selected_asset_for_chat['asset_id'])} · {_h(selected_asset_for_chat['name'])}</div>
              <div class='lp-panel-copy'>{_h(selected_asset_for_chat['line'])} · {_h(selected_asset_for_chat['status'])} · {_h(selected_asset_for_chat['failure_mode'])}</div>
            </div>""",
            unsafe_allow_html=True,
        )
    with right:
        _metric_card(
            "Planning horizon",
            _days_text(selected_asset_for_chat["days_remaining"]),
            "Current decision-support estimate",
            "◇",
        )
